  ...
  function calculatePrice() {
    ...
    for (let slotId of selectedSlot) {
      for (let priceObj of prices) {
        if (
          priceObj.slotId == slotId &&
          priceObj.tableType == selectedTableType
        ) {
      ...
      >
        ...
        <FormControl isRequired mb="20px">
          ...
          >
            ...
            <option value={dateArr[1]}>Ngày mai, {dateArr[1]}</option>
            <option value={dateArr[2]}>Ngày mốt, {dateArr[2]}</option>
          </Select>
        </FormControl>
          ...
          >
            {Object.entries(slotMap).map(([key, value]) => (
              <GridItem key={key}>
                <Button
                  isDisabled={
                    (selectedDate === dateArr[0] && value < allowedHour) ||
                    !checkIsTimeHasPrice(parseInt(key))
                  }
                  size="lg"
